...
private:
    ...
    typedef FP<void, MessageData*> messageHandlerFP;
...
int MQTT::Client<Network, Timer, a, MAX_MESSAGE_HANDLERS>::deliverMessage(MQTTString* topic, Message* message)
{
    ...
    MessageData handlerData = { message, topic->lenstring.data, topic->lenstring.len };
    ...
    for (int i = 0; i < MAX_MESSAGE_HANDLERS; ++i)
    {
        if (messageHandlers[i].topic != 0 && MQTTPacket_equals(topic, (char*)messageHandlers[i].topic))
        {
            ...
            {
                messageHandlers[i].fp(&handlerData);
                rc = SUCCESS;
                break;
            }
        }
    ...
    {
        defaultMessageHandler(&handlerData);
        rc = SUCCESS;
    }
...
int MQTT::Client<Network, Timer, MAX_MQTT_PACKET_SIZE, b>::keepalive()
{
    ...
    if (ping_timer.expired())
    {
        ...
        {
            // Return a failure if the ping timer has expired again
            // and we still have a ping outstanding.  This means we
            // never got a PINGRESP and are probably not connected.
            rc = FAILURE;
        }
    }
